...
* The [terraform-credentials-env credentials helper](https://github.com/apparentlymart/terraform-credentials-env) has been implemented as a Terraform feature. Users of terraform-credentials-env should note that the TF_TOKEN_* variables now take priority over CLI config credentials blocks created by `terraform login` for like host names.
